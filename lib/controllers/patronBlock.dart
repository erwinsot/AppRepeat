...
class Block {
  ...
  findCheckWords() async {
    final List<Map<String, dynamic>> mapWords =
        await _database.rawQuery("SELECT rowid, word, answer FROM words ");
    checkwordsdata = List.generate(mapWords.length, (index) {
      return CheckWordsData(mapWords[index]["rowid"], mapWords[index]["word"],
  ...
  Future<void> insertWords(CheckWords words) async {
    await _database.insert("words", words.toMap(),
